...
const escapeEnvVarCmd = (value) => {
  // The returned value needs to be enclosed in double quotes
  // Make sure it's a string
  // Escape ^ -> ^^
  // Escape & -> ^&
...
const setEnvVarCmd = (value, name) => {
  return `set "${name}=${escapeEnvVarCmd(value)}"`
}

// Exported for tests
exports.createLaunchScript = (command, argv, environment) => {
  const [ setEnvVarFn, escapeFn ] = os.platform() === 'win32' ? [ setEnvVarCmd, escapeParamCmd ] : [ setEnvVarSh, escapeSh ]
  const lines = _.map(environment, setEnvVarFn)
...
const elevateScriptWindows = async (path) => {
  ...
  const cmd = [ 'cmd', '/c', escapeParamCmd(path) ]
